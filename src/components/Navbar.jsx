...
export default function Navbar({ onOpenClarityModal }) {
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled 
          ? 'bg-efi-dark/90 backdrop-blur-md border-b border-white/10 py-3 shadow-xl' 
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <a href="#portal-preview" className="text-sm text-efi-gold/90 hover:text-efi-gold font-medium transition-colors flex items-center gap-1.5 bg-efi-gold/10 px-3 py-1 rounded-full border border-efi-gold/20">
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-efi-surface border-b border-white/10 px-4 pt-4 pb-6 space-y-4">
          <a 
            href="#diferencial" 
          ...
          >
            Portal de Clientes (Próximamente)
          </a>
          <a 
            href="#autoridad" 
            onClick={() => setMobileMenuOpen(false)}
